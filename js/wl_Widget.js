/*----------------------------------------------------------------------*/
/* wl_Widget v 1.0 by revaxarts.com
	...
	return this.each(function () {
		...
		if (!$this.data('wl_Widget')) {
			...
			$widgets.each(function () {
				...
				// deal with sortable objects.
				if (_opts.sortable) $widget.addClass("sortable");
				...
				.delegate('a.collapse', 'click.wl_Widget touchstart.wl_Widget', function (event) {
				...
				.delegate('a.reload', 'click.wl_Widget touchstart.wl_Widget', function (event) {
					var _opts = $widget.data('wl_Widget') || _opts,
						_content = $widget.find('div').eq(0);
			...
			if ($maincontent.data('wl_Widget').currentid++ >= $maincontent.data('wl_Widget').containercount) {
				...
				$container.each(function (i, cont) {
					...
					$.each(widgets, function (widget, options) {
						...
						(options.collapsed) ? _widget.addClass('collapsed').find('div').eq(0).hide().data('wl_Widget', {
							collapsed: true
